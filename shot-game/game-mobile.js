...
function createTarget() {
  ...
  while (!validPosition) {
    target.position.set(Math.random() * 100 - 50,  Math.random() * 100 -50, Math.random() * 100 - 50);
    validPosition = true;
  ...
  target.velocity = new THREE.Vector3((Math.random() - 0.5) * 0.15, (Math.random() - 0.5) * 0.15, (Math.random() - 0.5) * 0.15);
  targets.push(target);
  scene.add(target);
}
